...
export const addProduct = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!validationResult.success) {
      ...
      console.log("Validation failed on AddProduct");
      errors.forEach((err) => {
...
export const updateProduct = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // basically filter out invalid variation_id
    const incomingVariationIds = new Set(
      finalVariations.map((v) => v.variation_id).filter(Boolean),
    );

    // get the old variations that was removed, which mean didnt exist in newVariation
